...
export default function Items() {
  ...
  const bulkSerialMutation = useMutation({
    mutationFn: async ({ itemId, data }: { itemId: string; data: { quantity: number; startIndex: string; warehouseId: string } }) => {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <h2 className="text-lg font-semibold mb-2" data-testid="text-error-title">Error Loading Items</h2>
    ...
    <div className="min-h-screen bg-background pb-16">
      ...
      <main className="px-4 pt-4 space-y-4">
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            <Loader2 className="w-8 h-8 animate-spin text-muted-foreground" data-testid="loader-items" />
          </div>
          ...
          {items.map((item) => (
            ...
            >
              <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
                <CardTitle className="text-base font-medium" data-testid={`text-item-name-${item.id}`}>
                  {item.name}
                </CardTitle>
                {canEdit && (
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <span className="font-mono" data-testid={`text-item-sku-${item.id}`}>{item.sku}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Type: </span>
                    <span className="capitalize" data-testid={`text-item-type-${item.id}`}>{item.type}</span>
                  </div>
                  {item.category && (
                    <div>
                  ...
                  {item.description && (
                    <p className="text-muted-foreground text-xs mt-2" data-testid={`text-item-description-${item.id}`}>
                      {item.description}
                    </p>
                  )}
                </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-medium mb-2" data-testid="text-empty-title">No Items Yet</h3>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto" data-testid="dialog-item-form">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? "Update the item details below." : "Add a new item to your inventory. SKU will be auto-generated if left empty."}
            </DialogDescription>
          </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g., Dell Laptop" data-testid="input-item-name" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="Auto-generated if empty" data-testid="input-item-sku" />
                    </FormControl>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="serialized" data-testid="option-serialized">Serialized</SelectItem>
                        <SelectItem value="non-serialized" data-testid="option-non-serialized">Non-Serialized</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormDescription>
                      Serialized items have unique serial numbers (e.g., laptops). Non-serialized items are tracked in batches (e.g., pencils).
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="sellable" data-testid="option-sellable">Sellable</SelectItem>
                        <SelectItem value="loanable" data-testid="option-loanable">Loanable</SelectItem>
                        <SelectItem value="consumable" data-testid="option-consumable">Consumable</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormDescription>
                      Sellable items are marked as sold when scanned. Loanable items prompt for a user when scanned. Consumable items are marked out of stock when scanned.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} value={field.value ?? ""} placeholder="e.g., Electronics" data-testid="input-item-category" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea {...field} value={field.value ?? ""} placeholder="Item description..." rows={3} data-testid="input-item-description" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Minimum stock quantity before reordering (optional)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
                    ...
                    {!editingItem ? (
                      <div className="rounded-md bg-muted p-3">
                        <p className="text-xs text-muted-foreground">
                          Save this item first, then you can generate multiple serial numbers with QR codes at once.
                        </p>
                      </div>
                    ) : (
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select value={bulkSerialWarehouse} onValueChange={setBulkSerialWarehouse}>
                            <SelectTrigger id="bulk-warehouse" data-testid="select-bulk-warehouse">
                              <SelectValue placeholder="Select warehouse" />
                            </SelectTrigger>
                            <SelectContent>
                              {warehouses?.map((warehouse) => (
                                <SelectItem key={warehouse.id} value={warehouse.id} data-testid={`option-warehouse-${warehouse.id}`}>
                                  {warehouse.name}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Serial numbers will be: {bulkSerialStartIndex || editingItem.sku}-0001, {bulkSerialStartIndex || editingItem.sku}-0002, ...
                          </p>
                        </div>
      ...
      <AlertDialog open={!!deleteItem} onOpenChange={(open) => !open && setDeleteItem(null)}>
        <AlertDialogContent data-testid="dialog-confirm-delete">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="button-cancel-delete" onClick={() => setDeleteItem(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <Dialog open={!!bulkSerialResult} onOpenChange={(open) => !open && setBulkSerialResult(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto" data-testid="dialog-bulk-serial-success">
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="rounded-md bg-muted p-4 print:bg-white">
              ...
              <div className="grid grid-cols-3 gap-4 max-h-96 overflow-y-auto print:grid-cols-4 print:max-h-none print:gap-6">
                {bulkSerialResult?.serials.map((serial) => (
                  <div key={serial.id} className="flex flex-col items-center gap-2 p-3 bg-background rounded print:break-inside-avoid print:bg-white print:border print:border-gray-300" data-testid={`qr-code-${serial.id}`}>
                    {serial.qrToken && (
                      <QRCodeSVG 
                        value={serial.qrToken} 
                        size={120} 
                        level="M"
                      />
                    )}
                    <p className="text-xs font-mono text-center break-all" data-testid={`text-serial-${serial.id}`}>
                      {serial.serialNumber}
                    </p>
                  </div>
                ))}
            ...
            <p className="text-xs text-muted-foreground print:hidden">
              Click "Print QR Codes" to print these QR codes. You can also view and manage these serial numbers in the Inventory section.
            </p>
          </div>
